...
/* Set up */
    ...
    var colorsTitle = legend.append("div").html('<h3>Legend</h3>');
    var eventColors = legend.append("div").html('<div><span class="dot event"></span><span class="text">Event</span></div>');
    var compositionColors = legend.append("div").html('<div><span class="dot composition"></span><span class="text">Composition</span></div>');
    var manuscriptColors = legend.append("div").html('<div><span class="dot manuscript"></span><span class="text">Manuscript</span></div>');
    var editionColors = legend.append("div").html('<div><span class="dot edition"></span><span class="text">Edition</span></div>');
    var halfRight = legend.append("div").html('<div><span class="halfRight"></span><span class="text">Start Date</span></div>');
    var halfLeft = legend.append("div").html('<div><span class="halfLeft"></span><span class="text">End Date</span></div>');
    var dashedLine = legend.append("div").html('<div><span class="text dashed">Circa</span></div>');
    /* Add lines solid, dotted */
    ...
    var zoomControls = legend.append("div").html('<h3>Zoom</h3><button id="zoom-in" class="btn">+</button><button id="zoom-out" class="btn"> - </button>');
    var colorCodes = legend.selectAll("colors")
      .data(keys)
      .enter()
      .append("g")
        .attr("x", 25)
    ...
    colorCodes.append("circle")
        .attr("cx", 25)
        .attr("cy", function(d,i){ return 100 + i*25})
        .attr("r", 7)
        .style("fill", function(d){ return color(d)})
        
    colorCodes.append("text")
        .attr("x", 40)
        .attr("y", function(d,i){ return 100 + i*25}) 
    ...
    var yScale = d3.scaleLinear()
        .domain([0, yDomain[1]])
        .range([height, (margin.top + 75)]).nice();

    /* Axes components */
    /* --------------- */
    var xAxis = d3.axisTop(xScale).tickSize(10).tickFormat(d3.timeFormat("%Y")).ticks(20);
    
    var xAxisBackground = d3.axisTop(xScale).tickSize(-(margin.top + height * + 3), 0, 0).tickFormat("").ticks(20);
    ...
    xAxisDraw.attr('transform', 'translate(0, ' + margin.top + ')');
    xAxisBackgroundDraw.attr('transform', 'translate(0, ' + margin.top + ')');
    ...
    var link = gNodes.append("g")
        ...
        .attr("marker-end", function (d) {
            ...
            } else if (d.linkType === 'solidNoArrow') {
                return 'url(#arrowhead)'
            }
        });
    ...
    function zoomed() {
        ...
        graph.nodes.forEach (function (nodePos, i) {
            return nodePos.y = yScaleNew(nodePos.level);
        })
